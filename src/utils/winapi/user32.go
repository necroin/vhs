...
func SendInput(nInputs uint32, pInputs unsafe.Pointer, cbSize uintptr) (int32, error) {
	ret, _, err := procSendInput.Call(
		uintptr(nInputs),
		uintptr(pInputs),
		uintptr(cbSize))
	if ret == 0 {
		return 0, fmt.Errorf("[SendInput] failed: %s", err)
	}
...
func GetSystemMetrics(nIndex int32) (int32, error) {
	...
	if ret == 0 {
		return 0, fmt.Errorf("[GetSystemMetrics] failed: %s", err)
	}
	return int32(ret), nil
}
